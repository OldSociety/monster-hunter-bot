...
async function validateAllUserScores() {
  try {
    ...
    for (const user of users) {
      ...
      const userMonsters = await Collection.findAll({ where: { userId: user.user_id } });
      ...
      monstersWithStyle.forEach(m => {
        console.log(`   - ${m.name}: CR ${m.cr}, m_score ${m.m_score}, type ${m.type} (style ${m.style})`);
      });
      
      // Calculate overall top 3 score.
      const overallSorted = monstersWithStyle.slice().sort((a, b) => (b.cr - a.cr) || (b.m_score - a.m_score));
      const overallTop3 = overallSorted.slice(0, 3);
      const overallCalculatedScore = overallTop3.reduce((acc, m) => acc + m.m_score, 0);
      ...
      for (const style in styleGroups) {
        const sortedByStyle = styleGroups[style].slice().sort((a, b) => (b.cr - a.cr) || (b.m_score - a.m_score));
        ...
        if (storedScore !== styleScore) {
          console.log(`\n[MISMATCH] User ${user.user_id} ${style.charAt(0).toUpperCase() + style.slice(1)} Score: stored = ${storedScore}, calculated = ${styleScore}`);
