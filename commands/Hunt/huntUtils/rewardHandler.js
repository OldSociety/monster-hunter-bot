...
async function addRewardToUser(user, goldAmount = 0, tokenAmount = 0) {
  ...
  console.log(`💰 Rewarding User ${user.user_id}: +${goldAmount} Gold, +${tokenAmount} Tokens`);
  await user.save();
}
...
async function displayHuntSummary(interaction, user, huntData, levelCompleted) {
  ...
  if (!huntData.level) {
    console.error(`❌ ERROR: huntData.level is undefined in displayHuntSummary.`);
  ...
  if (!currentHunt) {
    console.error(`❌ ERROR: Hunt data not found for ID ${huntId} on page ${pageKey}`);
  ...
  currentHunt.battles.forEach((battle) => {
    if (battle.type === 'mini-boss' && !user.completedHunts.includes(currentHunt.key)) {
  ...
  const summaryEmbed = new EmbedBuilder()
    ...
    .setDescription(`**Gold Earned:** 🪙${totalGoldEarned}\n**Tokens Earned:** 🧿${totalTokensEarned}`)
    .setColor('#FFD700');
...
if (huntData.level.id > user.completedLevels) {
  console.log(`📈 Increasing completedLevels from ${user.completedLevels} to ${huntData.level.id}`);
  user.completedLevels = huntData.level.id;
  await user.save();
}

    if (currentHunt.unlocks) {
      const nextHunt = huntPages[pageKey].hunts.find((hunt) => hunt.id === currentHunt.unlocks);
    ...
    if (currentHunt.unlocksPage && huntPages[currentHunt.unlocksPage]) {
      ...
      summaryEmbed.addFields({
        ...
        value: `You have unlocked **${huntPages[currentHunt.unlocksPage].name}**!`,
      });
    }
  }

  summaryEmbed.setThumbnail(interaction.user.displayAvatarURL({ format: 'png', size: 128 }));
