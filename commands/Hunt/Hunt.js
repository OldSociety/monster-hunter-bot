...
module.exports = {
  ...
  async execute(interaction) {
    console.log(`Hunt command executed by: ${interaction.user.tag} (ID: ${interaction.user.id})`);
    ...
    collector.on('collect', async (i) => {
      ...
      if (i.customId === 'hunt_select' || i.customId === 'use_ichor' || i.customId === 'cancel_hunt') {
        console.log(`Ignoring interaction ${i.customId} as it's already handled.`);
        return;
      }
    ...
    collector.on('end', (collected, reason) => {
      console.log(`Collector ended. Reason: ${reason}. Interactions collected: ${collected.size}`);
    });
  },
};
