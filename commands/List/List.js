...
async function loadMonsterCache() {
  ...
  if (!module.cachePopulated || Object.keys(module.monsterCacheByTier).length === 0) {
...
module.exports = {
 ...
 if (!cacheMonstersByTier || Object.keys(cacheMonstersByTier).length === 0) {
  ...
  if (firstMonster) {
    console.log("[CACHE] First Monster Loaded:", JSON.stringify(firstMonster, null, 2));
  } else {
...
Object.values(cacheMonstersByTier).flat().slice(0, 3).forEach((monster, index) => {
  console.log(
    `  [${index + 1}] Name: ${monster.name}, CR: ${monster.cr}, Type: ${monster.type}, HP: ${monster.hp}`
  );
});


let filteredMonsters = Object.values(cacheMonstersByTier).flat().filter(
  (monster) =>
    monster.challenge_rating >= minCR &&
    monster.challenge_rating <= maxCR
);
console.log("CacheMonstersByTier:", cacheMonstersByTier);
console.log("First Monster in Cache:", Object.values(cacheMonstersByTier)?.[0]?.[0]);
...
console.log(Object.values(cacheMonstersByTier).flat().slice(0, 1));
        ...
        console.log(`🔹 Input Values - monsterType: ${monsterType}, cr: ${crFilter}, hp: ${hp}, hpRange: ${hpRange}`);
        ...
        .map(monster => ({
          ...
          console.log("🔹 First 3 monsters (raw data):", monstersToDisplay.slice(0, 3));
          console.log("🔹 Checking first 3 monsters from cache:");
  monstersToDisplay.slice(0, 3).forEach((monster, index) => {
    console.log(`  [${index + 1}] Name: ${monster.name}, CR: ${monster.cr}, Type: ${monster.type}, HP: ${monster.hp}`);
  });
        ...
        if (monsterType) {
          ...
          monstersToDisplay = monstersToDisplay.filter(monster => monster.type?.toLowerCase() === monsterType);
          console.log(`✅ After type filter (${monsterType}): ${monstersToDisplay.length} monsters`);
        }
        ...
        if (crFilter?.toLowerCase() === 'all') {
          ...
          const crEntries = Object.entries(crCounts).sort((a, b) => parseFloat(a[0]) - parseFloat(b[0]));
          let currentPage = 0;
      
          const createCRCountEmbed = (page) => {
            const paginatedCRs = crEntries.slice(page * monstersPerPage, (page + 1) * monstersPerPage);
            const embed = new EmbedBuilder()
              .setTitle('Monster Count by CR')
              .setColor(0x00bfff)
              .setFooter({
                text: `Page ${page + 1} of ${Math.ceil(crEntries.length / monstersPerPage)}`,
              });
          ...
          if (!isNaN(parsedCR)) {
            monstersToDisplay = monstersToDisplay.filter(monster => monster.cr === parsedCR);
            console.log(`✅ After CR filter (CR ${parsedCR}): ${monstersToDisplay.length} monsters`);
          }
        }
        ...
        if (hp !== null) {
          ...
          monstersToDisplay = monstersToDisplay.filter(
            monster => monster.hp !== undefined && monster.hp >= (hp - hpRange) && monster.hp <= (hp + hpRange)
          );
        
          console.log(`✅ HP Filter: Before: ${beforeFilterCount}, After: ${monstersToDisplay.length}`);
        }
        ...
        const createEmbed = (page) => {
          const paginatedMonsters = monstersToDisplay.slice(page * monstersPerPage, (page + 1) * monstersPerPage);
          let embedTitle = '';
      
          if (monsterType) {
            embedTitle = `${monsterType.charAt(0).toUpperCase() + monsterType.slice(1)} Monsters (${monstersToDisplay.length})`;
          } else if (crFilter) {
            embedTitle = crFilter.toLowerCase() === 'all'
              ? 'All Monsters by CR'
          ...
          const embed = new EmbedBuilder()
            ...
            .setColor(0x00bfff)
            .setFooter({
              text: `Page ${page + 1} of ${Math.ceil(monstersToDisplay.length / monstersPerPage)}`,
            });
        ...
        const getRow = (page) => {
          return new ActionRowBuilder().addComponents(
            ...
            new ButtonBuilder()
              ...
              .setLabel('Next')
              .setStyle(ButtonStyle.Secondary)
              .setDisabled((page + 1) * monstersPerPage >= monstersToDisplay.length),
            new ButtonBuilder()
              .setCustomId('finish')
              .setLabel('Finish')
              .setStyle(ButtonStyle.Danger)
