  ...
  module.exports = {
    ...
    async execute(interaction) {
      ...
      if (existingSession) {
        ...
        const now = Date.now()
        if (now < finishTime) {
          const { formatTimeRemaining } = require('../../handlers/trainTimerHandler')
          ...
          return interaction.editReply({ embeds: [inProgressEmbed], components: [] })
        } else {
          ...
          collector.on('collect', async (i) => {
            ...
            try {
              ...
              if (i.customId === 'finish_training') {
                ...
                console.log('[finish_training] Retrieved training session:', trainingSession)
                if (!trainingSession) {
                  console.log('[finish_training] No active training session found.')
                  return i.editReply({
                ...
                const now = Date.now()
                console.log('[finish_training] finishTime:', finishTime, 'now:', now)
                if (now < finishTime) {
                  const { formatTimeRemaining } = require('../../handlers/trainTimerHandler')
                ...
                const currentBase = user.base_score || 0
                console.log('[finish_training] maxBase:', maxBase, 'currentBase:', currentBase)
                if (currentBase >= maxBase) {
                  console.log('[finish_training] User base score already maxed out.')
                  return i.editReply({
                ...
                console.log('[finish_training] Training monster record:', trainingMonster)
                if (trainingMonster) {
                ...
                if (allowedIncrease < trainingSession.bonus) {
                  ...
                  console.log('[finish_training] Applied extra bonus:', extraBonus)
                }
                ...
                console.log('[finish_training] Finishing training, sending finishEmbed.')
      ...
      if ((user.base_damage || 0) >= maxBase) {
        const embed = new EmbedBuilder()
          ...
          .setDescription(`Your base damage is already maxed out (${maxBase}). You cannot train further.`)
          .setColor('Blue')
      ...
      const trainExplanationEmbed = new EmbedBuilder()
        ...
        .setColor('Blue')
        .setFooter({
          text: `Your current base score: ${user.base_score || 0} / ${maxBase}`,
        })
