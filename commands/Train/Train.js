...
async function doubleCheckBaseScore(user) {
  const calculatedMax = await calculateMaxBaseScore(user.id)
  if (user.base_score > calculatedMax) {
    user.base_score = calculatedMax
    await user.save()
  }
}
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!user) return
    await doubleCheckBaseScore(user)
    ...
    // If there's an existing training session, handle it first.
    if (existingSession) {
      const finishTime =
      ...
      if (now < finishTime) {
        ...
        // Save the reply message and attach a collector.
          ...
          try {
            ...
            if (i.customId === 'finish_training') {
              ...
              if (currentBase >= maxBase) {
                ...
                return i.editReply({
                  content: `Your base score is already maxed out (${maxBase}). You cannot train further.`,
                  components: [],
                })
              }
